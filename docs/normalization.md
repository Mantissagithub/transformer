...
\operatorname{LN}(x) = \gamma \frac{x-\mu}{\sigma+\epsilon} + \beta
\]
...
\[
\operatorname{RMS}(x)=\sqrt{\frac{1}{d}\sum_i x_i^2+\epsilon},\quad
\operatorname{RMSNorm}(x)=g\frac{x}{\operatorname{RMS}(x)}
\]
